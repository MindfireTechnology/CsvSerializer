    ...
    public class Serializer : ISerializer
    {
        ...
        protected IEnumerable<PropertyData> GetProperties(object value, bool recursive, string prefix = "", int? colnum = null)
        {
            ...
            foreach (var prop in value.Properties())
            {
                ...
                // Check for StringLengthAttribute
                if (Settings.UserDataAnnotationAttributes && attributes.Any(n => n.TypeName() == "StringLengthAttribute"))
                {
                    var attribute = (StringLengthAttribute)attributes.First(n => n.TypeName() == "StringLengthAttribute");
                    object propertyValue = prop.GetValue(value, null);

                    if (propertyValue.ToString().Length > attribute.MaximumLength)
                    {
                        throw new CsvSerializationException(attribute.FormatErrorMessage(prop.Name), null);
                    }
                }
